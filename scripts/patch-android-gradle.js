...
function main() {
  ...
  tryPatch(expoGradlePlugin, (code) => {
    ...
    // Force stdlib family to 1.9.24 inside composite build to avoid 2.1.x on classpath
    if (!/org\.jetbrains\.kotlin:kotlin-stdlib:1\.9\.24/.test(out)) {
      out += `
...
`;
  ...
  tryPatch(expoAutolinkPlugin, (code) => {
    ...
    // Force stdlib family to 1.9.24 to avoid 2.1.x jars
    if (!/org\.jetbrains\.kotlin:kotlin-stdlib:1\.9\.24/.test(out)) {
      out += `
